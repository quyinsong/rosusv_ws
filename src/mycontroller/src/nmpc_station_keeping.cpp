...
//目标跟踪控制初始化函数
StationKeepingNMPC::StationKeepingNMPC(int predict_step, float sample_time)
{
...
StationKeepingNMPC::~StationKeepingNMPC()
{

}

//创建定点控制求解器
void StationKeepingNMPC::set_my_nmpc_solver_for_StationKeeping()
{
    //模型建立
...
void StationKeepingNMPC::opti_solution_for_StationKeeping(Eigen::Matrix<float, 6, 1> current_states,Eigen::Matrix<float,6,1> desired_states)
{
    //获取期望位置
...
Eigen::Matrix<float,2,1> StationKeepingNMPC::get_controls()        // casadi::SX c13 = 0;
        // casadi::SX c23 = 0;
{
...
std::vector<Eigen::Matrix<float, 6, 1>> StationKeepingNMPC::get_predict_trajectory()
{
